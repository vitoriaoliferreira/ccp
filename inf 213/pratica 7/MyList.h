...
public:
	...
	int eraseMatchingElements(const T&elementAux);
	void reverse();
	void reverseElements(Node<T> *dataAux);
...
private:
	...
	// void destroy(iterator primeiroElemen); // Destroy Recursivo (Funcao Auxiliar)
	void destroy();
	void create();
...
// template<class T>
// void MyList<T>::destroy(MyList<T>::iterator primeiroElemen) { 
// 	if (primeiroElemen == NULL) {
// 		dataSize = 0;
// 		return;
// 	}
// 	destroy(primeiroElemen->next);
// 	delete primeiroElemen;
// }
...
void MyList<T>::destroy() { 
	...
	while (dataFirst != NULL) {
		Node<T> *dataFirstAux = dataFirst->next;
		delete dataFirst;
		dataFirst = dataFirstAux;
	}
	dataSize = 0;
	return;	
...
void MyList<T>::push_front(const T&elem) {
	...
	if (dataFirst == NULL) {
		...
		Node<T> *dataFirstAux = dataFirst;

		dataFirst = new Node<T>(elem);
		dataFirst->next = dataFirstAux;
		dataSize++;
	}
...
template<class T>
int MyList<T>::eraseMatchingElements(const T&elementAux) {

	int qntdElemRemovidos = 0;

	iterator itAux = dataFirst;

	while (itAux != end()) {
		if (deref(itAux) == elementAux) {
			itAux = erase(itAux);
			qntdElemRemovidos++;
		}
		else {
	...
	return qntdElemRemovidos;

}

template<class T>
void MyList<T>::reverse() {
	reverseElements(dataFirst);
}

template<class T>
void MyList<T>::reverseElements(Node<T> *dataAux) {
	if (dataAux->next == NULL) {
		dataFirst = dataAux;
		return;
	}
	reverseElements(dataAux->next);
	Node<T> *dataAux2 = dataAux->next;
	dataAux2->next = dataAux;
	dataAux->next = NULL; 
}
